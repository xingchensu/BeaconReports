...
-- Description:	Drilldown Vs Prev Years Repo
-- =============================================,
--[dbo].[KeyStats_Repo_LoadDrillDownVsPrevYearsRepo] N'5/20/2015',N'12/20/2015', @CollectorGUID = 'EF41E23E-1CC1-E411-80CD-00155D01E300' '00000000-0000-0000-0000-000000000000'
CREATE PROCEDURE [dbo].[KeyStats_Repo_LoadDrillDownVsPrevYearsRepo] --N'5/20/2015',N'12/20/2015', @CollectorGUID = 'EF41E23E-1CC1-E411-80CD-00155D01E300'
...
BEGIN
    ...
    SET @LastYearSamePeriodTo = CAST(( YEAR(@BeginDate) - 1 ) AS VARCHAR(20))
	...
	--IF/ELSE deciding whether we are filtering by Salesman or Collector
	...
	IF OBJECT_ID('tempdb..#DrillDownStats') IS NOT NULL --Same temp table as in LoadCompanyRepo SP. Except the headers is different.
	...
	BEGIN
		...
		--REPO TIMELINE (days)
		declare @RT_AverageStartToRecovery decimal = isnull(
		(
			select avg(DATEdiff(day,cr.new_repostartDATE,cr.ActualPickupDATE))
			from #ClosedRepos cr
		),null);
		declare @RT_AverageRecoveryToComplete decimal = isnull(
		(
		...
		INSERT INTO #DrillDownStats
			...
			--ACTIVITY DATA
			null,null,null,null,null,null,null --Activity Data addition   
	...
	FROM  #DrillDownStats m1
	JOIN #DrillDownStats m2
	...
	--SELECT 0,@counter,'Difference',0,0,0,0,0,0,@fromDate,@toDate,
 --      COALESCE(
 --      (
	--	   SELECT TOP 1 CR_NumberOfRepos
	--	   FROM #DrillDownStats mi
	--	   WHERE mi.G_current_index > m.G_current_index
 --      ), 0) - CR_NumberOfRepos AS diff
	--FROM  #DrillDownStats m
	--WHERE m.G_current_index = 4
END

--[dbo].[KeyStats_Repo_LoadDrillDownRepo] N'5/20/2015',N'12/20/2015', @CollectorGUID = 'EF41E23E-1CC1-E411-80CD-00155D01E300'
GO
