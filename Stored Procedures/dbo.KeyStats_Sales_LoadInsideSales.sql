...
--				Sales View Data 
-- =============================================
--[KeyStats_Sales_LoadInsideSales] '1/01/2015','12/22/2015',null,null,null,'00000000-0000-0000-0000-000000000000'
CREATE PROCEDURE [dbo].[KeyStats_Sales_LoadInsideSales]
    @DateFrom DATETIME ,
    @DateTo DATETIME ,
...
AS 
    DECLARE @MiscInsideSalesID AS UNIQUEIDENTIFIER
    SET @MiscInsideSalesID = '00000000-0000-0000-0000-000000000000'

  --Category ID
    DECLARE @categoryID AS INT
    SET @categoryID = 2--fixed for sales
  ...
  --select * from #employees
    ...
    DECLARE @insideSales AS VARCHAR(160)
    IF @insideSalesID IS NOT NULL 
        BEGIN
            SELECT  @insideSales = fullname_group
            FROM    #employees
            WHERE   CRMGuid_group = @insideSalesID
        END
    ...
    CREATE TABLE #details
        (
          ...
          OtherIncomeExpense MONEY NULL,
                    [insideSalesid_group] [uniqueidentifier] NULL 
        )
  ...
  -- ********** SELECT Sales Details **********
   SELECT  name ,
                    ...
                    [consultant] AS Owner
            FROM    #details
            WHERE  insideSalesid_group=@insideSalesID OR @insideSalesID IS NULL
  ...
  --CREDIT APPROVAL section
    ...
    INSERT  INTO #reviewedDeals
            ...
            FROM    [dbo].[KeyStats_ReviewedTerms_HourlySnapshot] s
                    INNER JOIN #employees em ON s.insideSalesId = em.CRMGuid
    ...
    INSERT  INTO #fund
            ...
            FROM    ( SELECT    *
                    ...
                    INNER JOIN #employees em ON a.insidesalesid = em.CRMGuid
            GROUP BY em.username_group
                    WITH ROLLUP
  ...
  --open lead 
    ...
    INSERT  INTO #OpenLeads
            ...
            FROM    [KeyStats_OpenLeadPipeline_DailySnapshot] s
                    INNER JOIN #employees em ON em.crmguid = s.insidesalesid
            WHERE   SnapshotDate = @dateSnapshot
                    AND ( consultantid = @outsideSalesID
                          OR @outsideSalesID IS NULL
                        )
  ...
  --- Call quality
    ...
    INSERT  INTO #callquality
            ...
            FROM    #employees em
                    INNER JOIN [Intranet_Beaconfunding].[dbo].[CallCenter_ActionAgentInfo] Users ON Users.bfcuserid = em.userid
                                                              AND em.IntranetTable = 'Beacon'
                    INNER JOIN [CRMReplication].[dbo].[Noble_Maestro_nsc_recordings] R ON r.tsr = Users.nobleAgentID
                    INNER JOIN [CRMReplication].[dbo].[Noble_Recording_Evaluations] E ON E.record_id = r.record_id
            WHERE   E.createdon >= @datefrom
                    AND E.createdon <= @dateto
 ...
 --     ELSE 'BFC Tot.'
 --   END AS username_group
 --    ,     SUM(expenseAmount) AS TotalExpense INTO #TinyExpense
 -- FROM 
 -- #employees  em
 -- INNER JOIN 
 -- intranet_beaconfunding.dbo.Exp_Reports r
 --   ON r.UserID = em.userid AND  em.IntranetTable = 'Beacon'
 --LEFT join   
 -- intranet_beaconfunding.dbo.Exp_Expenses ex
 --   ON r.ReportID = ex.ReportID    
 -- LEFT JOIN intranet_beaconfunding.dbo.Exp_ExpenseTypes t
 --   ON ex.ExpenseTypeID = t.ExpenseTypeID
 -- LEFT JOIN intranet_beaconfunding.dbo.Exp_StatusReasons s
 --   ON r.StatusReasonId = s.StatusReasonId
...
-- ********** Company Sales **********
    ...
    WITH    cte_accepted
        ...
        SELECT DISTINCT
                ...
                cq.CallQuality AS CallQualityRatingVal ,
                cq.TotalCallsEvaluated AS TotalCallsEvaluatedVal ,
                0 AS _sc_excluded
        INTO    #final
        FROM    #employees emp
    ...
    --ON te.username_group=emp.username_group
                ...
                LEFT JOIN #callquality cq ON cq.username_group = emp.username_group     
  ...
  --  ON te.username_group = emp.username_group
    ...
    SELECT  @em_count = COUNT(CRMGuid) - 1--exclude bfc total
    FROM    #employees
    ...
    INSERT  INTO #final
            ( [Index] ,
              ...
              TotalKeystrokesVal ,
              TotalEmailsVal ,
              CallQualityRatingVal ,
              TotalCallsEvaluatedVal ,
              _sc_excluded
            )
            SELECT  1 , -- Index - int
                    ...
                    CallQualityRatingVal ,
                    TotalCallsEvaluatedVal / @em_count ,
                    1
            FROM    #final
            WHERE   [index] = 0
    ...
    ELSE 
        BEGIN
            SELECT  CASE WHEN ConsultantID = @insideSalesID THEN 0 ELSE [index] END AS [Index] ,
                    ConsultantUserName ,
                    ConsultantName ,
                    ConsultantID ,
                    ConsultantFullName ,
            ...
            SELECT  2 ,
                    ...
                    fi.CallQualityRatingVal - fa.CallQualityRatingVal ,
                    fi.TotalCallsEvaluatedVal - fa.TotalCallsEvaluatedVal
            FROM    #final fi
                    INNER JOIN #final fa ON fi.ConsultantID = @insideSalesID
...
--KeyStats_Sales_LoadinsideSales '1/01/2015','12/22/2015'
GO
